...
namespace Semver
{
    ...
    /// From: https://github.com/maxhauser/semver/blob/master/Semver/SemVersion.cs 
    /// Authored by: https://github.com/maxhauser
    /// </summary>
    [Serializable]
    public sealed class SemVersion : IComparable<SemVersion>, IComparable, ISerializable
    {
        readonly static Regex _parseRegEx =
            new Regex( @"^(?<major>\d+)" +
                @"(\.(?<minor>\d+))?" +
                @"(\.(?<patch>\d+))?" +
                @"(\-(?<pre>[0-9A-Za-z\-\.]+))?" +
        ...
        {
            var match = _parseRegEx.Match( version );
            if( !match.Success )

...
namespace SimpleGitVersion
{
    public sealed partial class ReleaseTagVersion
    {
        ...
        public static ReleaseTagVersion TryParse( string s, bool analyseInvalidTag = false )
        {
            ...
            if( !m.Success )
            {
                if( analyseInvalidTag )
                {
                    ...
                    if( m.Success ) return new ReleaseTagVersion( s, true, SyntaxErrorHelper( s, m ) );
                }
        ...
        static Regex _regexApproxSuffix = new Regex( @"^(-(?<1>.*?))?(\+(?<2>.*))?$", RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.ExplicitCapture );
